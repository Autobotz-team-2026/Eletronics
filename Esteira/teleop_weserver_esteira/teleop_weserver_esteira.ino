...
void Task_Control(void *pvParameters) {
  ...
  digitalWrite(R_EN_M2, HIGH); // Ativa o driver
  digitalWrite(L_EN_M2, HIGH); // Ativa o driver
  ledcAttach(RPWM_M2, PWM_FREQ, PWM_RES);
  ledcAttach(LPWM_M2, PWM_FREQ, PWM_RES);
  ledcWrite(RPWM_M2, 0);
  ledcWrite(LPWM_M2, 0);
  ...
  while (1) {
    ...
    // WATCHDOG: Desliga tudo se perder comunicação
    if ((millis() - time_local) > WATCHDOG_TIMEOUT_MS) {
      ...
      // Controle Motor 1 (Esquerda) - L298N
      if (v_left > 0) { 
      ...
      // Controle Motor 2 (Direita) - BTS7960
      if (v_right > 0) { 
        ledcWrite(LPWM_M2, 0); 
        ledcWrite(RPWM_M2, pwm_right); 
      }
      else if (v_right < 0) { 
        ledcWrite(RPWM_M2, 0); 
        ledcWrite(LPWM_M2, pwm_right); 
      }
      else { 
        ledcWrite(RPWM_M2, 0); 
        ledcWrite(LPWM_M2, 0); 
...
void Task_WebServer(void *pvParameters) {
 // Configura o ESP32 para criar a própria rede (Access Point)
  ...
  Serial0.println();
  Serial0.println(">> [WEB] Ponto de acesso Wi-Fi criado!");
  Serial0.print(">> [WEB] IP para acessar no navegador: http://"); 
